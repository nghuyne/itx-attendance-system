...
class AttendanceHybridValidationIntegrationTest extends AbstractIntegrationTest {
    ...
    @Nested
    @DisplayName("Mobile path: bssid present → validate against valid_macs")
    class CheckInMobilePath {
    ...
    @Nested
    @DisplayName("Web path: bssid null/blank → validate against valid_ips (unchanged)")
    class CheckInWebPath {
    ...
    @Nested
    @DisplayName("Client site mode bypasses both checks entirely")
     ...
     */
    @Nested
    @DisplayName("Check-out mirrors the check-in validation path (Epic 10 P0 fix)")
    class CheckOutMirrorsCheckIn {
